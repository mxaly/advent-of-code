...
print = CPU.draw(path) # > Enum.group_by(fn({{x, y}, _}) -> y end) |> Enum.map(fn({y, row}) -> {y, Enum.sort_by(row, fn({{x,_},_}) -> x end)} end) |> Enum.sort_by(fn {k, _v} -> -k end) |> Enum.map(fn {k, rows} -> Enum.map(rows, fn{{_,_}, c} -> c end) |> Enum.join(",") end) |> Enum.join("\n")
IO.puts(print)
