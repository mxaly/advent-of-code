defmodule CPU do
  ...
  defp save(memory, [position | _rest], config, pointer, base) do
    ...
    IO.puts("saving... at #{position + base}, value: #{input}, base: #{base}, opt: #{position}, config: #{Enum.join(config)}")
    position = if(at(config, 0) == 2) do position + base else position end
    memory = memory |> replace_at(position, input)
    {memory, pointer + 2}
  end

  def print(memory, [position | _rest], config, pointer, base) do
  ...
  def jump_if_true(memory, [read, position | _rest], config, pointer, base) do
  ...
  def jump_if_false(memory, [read, position | _rest], config, pointer, base) do
    if get_arg(memory, {read, at(config, 0), base}) == 0 do
      {memory, get_arg(memory, {position, at(config, 1), base})}
      # {memory, position}
    else
  ...
  defp increase_base(memory, [arg | _rest], config, base) do
  ...
  def tick({memory, pointer}, base) do
    ...
    case(parse_opp(Integer.digits(opp))) do
      ...
      {3, config} -> tick(save(memory, code, config, pointer, base), base)
      {4, config} -> tick(print(memory, code, config, pointer, base), base)
      {5, config} -> tick(jump_if_true(memory, code, config, pointer, base), base)
      {6, config} -> tick(jump_if_false(memory, code, config, pointer, base), base)
      {7, config} -> tick(less_than(memory, code, config, pointer, base), base)
      {8, config} -> tick(equals(memory, code, config, pointer, base), base)
      {9, config} -> tick({memory, pointer + 2}, increase_base(memory, code, config, base))
