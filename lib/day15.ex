defmodule Drone do
  ...
  def step(program, {base, args}, {x, y}, %{arg: direction, x: dx, y: dy}, map) do
    ...
    case tick(program, base, [direction]) do
      ...
      {:out, {memory, pointer, out}, base, args} ->
        ...
        case out do
          ...
          1 ->
            # IO.puts("out: #{out}, x:#{x + dx}, y:#{y + dy} = . |-> #{length(get_unexplored(screen, new_cords))}")
            sequence(
              {memory, pointer},
              {base, args},
              new_cords,
  ...
  def sequence(input) do
    start_cords = {0, 0}
    map = %{{0, 0} => "S"}
    sequence({input, 0}, {0, []}, start_cords, get_unexplored(map, start_cords), map)
  end

  def sequence(_program, _config, _current_point, [], map) do
    IO.puts("dead end")
    map
  end

  def sequence(program, config, current_point, directions, map) do
    {x, y} = current_point
    Enum.map(directions, fn direction ->
      step(program, config, current_point, direction, map)
    end)
  end
  ...
  def run() do
    ...
    map = sequence(input) |> flat_and_merge |> Enum.uniq |> Map.new
    # fill_oxygen(map)
  end
  ...
  def print(map) do
    ...
    Enum.map(min_y..max_y, fn(y) ->
      Enum.map(min_x..max_x, fn(x) ->  map[{x, y}] || "W" end) |> Enum.join()
    end) |> Enum.join("\n")
  end
end
