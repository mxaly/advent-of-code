defmodule CPU do
  ...
  defp get_arg(memory, {arg, 0}) do at(memory, arg) end
  defp get_arg(memory, {arg, nil}) do at(memory, arg) end
  defp get_arg(_, {arg, 1}) do arg end
  ...
  defp less_than(memory, [a, b, position | _rest], config, pointer) do
    memory = if(get_arg(memory, {a, at(config, 0)}) < get_arg(memory, {b, at(config, 1)})) do
  ...
  def tick({memory}) do tick({memory, 0}) end
